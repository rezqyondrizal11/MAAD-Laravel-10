        ...
        if ($update) {
            ...
            if ($request->hasFile('foto_profil')) {
                ...
                if ($update->foto_profil == '') {
                    if (storage_path('app/public/uploads/profil')) {
                        if ($ext == 'png' || $ext == 'jpg' || $ext == 'jpeg') {
                            $ext = $request->file('foto_profil')->extension();
                            $final = 'profil' . time() . '.' . $ext;

                            $request->file('foto_profil')->move(storage_path('app/public/uploads/profil/'), $final);
                            $update->foto_profil = $final;
                        }
                    }
                } elseif ($update->foto_profil) {
                    unlink(storage_path('app/public/uploads/profil/' . $update->foto_profil));
                    if ($ext == 'png' || $ext == 'jpg' || $ext == 'jpeg') {
                        $ext = $request->file('foto_profil')->extension();
                        $final = 'profil' . time() . '.' . $ext;
